...
# Setup logging
logging.basicConfig(level=logging.INFO, filename='sugar_procurement_chatbot.log', format='%(asctime)s - %(levelname)s - %(message)s')
...
def load_coord_cache():
    try:
        ...
        logger.error(f"Failed to load coord_cache: {e}")
        return {}
...
def save_coord_cache(cache):
    try:
        ...
        logger.error(f"Failed to save coord_cache: {e}")
...
def load_data(file_path='Sugar Data.xlsx'):
    ...
    try:
        if not os.path.exists(file_path):
            error = f"Data file not found at: {file_path}"
            logger.error(error)
        ...
        print("Raw Product Names:", sorted(df['Product Name'].unique()))
        print("Sample Product Names:", df['Product Name'].head().tolist())
        ...
        print("Normalized Products:", sorted(df['Normalized Product'].unique()))
...
def update_output(submit_n_clicks, coords_n_clicks, sort_by, download_n_clicks, requirement, latitude, longitude, stored_data):
    ...
    if df.empty:
        logger.error("No data available")
        return [
            html.P("Error: No data loaded from 'Sugar Data.xlsx'", className="text-red-600"),
            "",
            False,
            "modal-content hidden",
            None,
...
if __name__ == '__main__':
    print("Starting Dash app locally...")
    logger.info("Starting Dash app locally")
    try:
        app.run_server(debug=True)
    except Exception as e:
        error_msg = f"Failed to run Dash app: {str(e)}\n{''.join(traceback.format_tb(e.__traceback__))}"
        print(error_msg)
        logger.error(error_msg)
        sys.exit(1)
